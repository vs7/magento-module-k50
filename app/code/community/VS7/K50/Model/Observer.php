...
class VS7_K50_Model_Observer
{
    ...
    {
        if (empty(Mage::getStoreConfig('k50/general/active'))) {
            return;
        }

        $this->_baseUrl = Mage::app()->getStore($this->_storeId)->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK);

        $feedPath = Mage::getBaseDir('media') . DS . 'k50' . DS . Mage::getStoreConfig('k50/general/filename') . '.yml';
        ...
        $this->_row('<shop>');
        $this->_row('<name>' . Mage::getStoreConfig('k50/general/store_name') . '</name>');
        $this->_row('<company>' . Mage::getStoreConfig('k50/general/company_name') . '</company>');
    ...
    private function _writeProductsData()
    {
        ...
        for ($i = 0; $i < $productCollectionSize;) {
            ...
            foreach ($productCollection as $product) {
                $this->_row('<offer id="' . $product->getId() . '" available="true">');
...
//                foreach (Mage::helper('vs7_k50')->filterAndNameProductData($product) as $param) {
...
//                }
    ...
    private function _getTempProductsPath()
    {
        if (empty($this->_productsTmpFile)) {
            $this->_productsTmpFile = tempnam(sys_get_temp_dir(), 'vs7_k50_products_');
        }
    ...
    private function _getTempCategoriesPath()
    {
        if (empty($this->_categoriesTmpFile)) {
            $this->_categoriesTmpFile = tempnam(sys_get_temp_dir(), 'vs7_k50_categories_');
        }
